...
import ruleColOriginLabel from '@salesforce/label/c.GestaoSLA_RuleColOrigin';
...
import ruleColValidityLabel from '@salesforce/label/c.GestaoSLA_RuleColValidity';
...
import ruleScopeGlobalLabel from '@salesforce/label/c.GestaoSLA_RuleScopeGlobal';
import ruleScopeByCategoryLabel from '@salesforce/label/c.GestaoSLA_RuleScopeByCategory';
import ruleScopeByInternalAreaLabel from '@salesforce/label/c.GestaoSLA_RuleScopeByInternalArea';
import ruleFilterOriginAllLabel from '@salesforce/label/c.GestaoSLA_RuleFilterOriginAll';
...
import validityOpenLabel from '@salesforce/label/c.GestaoSLA_ValidityOpen';
import validityNoStartLabel from '@salesforce/label/c.GestaoSLA_ValidityNoStart';
import validityNoEndLabel from '@salesforce/label/c.GestaoSLA_ValidityNoEnd';
import validityUntilLabel from '@salesforce/label/c.GestaoSLA_ValidityUntil';
...
import errorRuleValidityEndBeforeStartLabel from '@salesforce/label/c.GestaoSLA_ErrorRuleValidityEndBeforeStart';
...
const RULE_SCOPE_GLOBAL = 'Global';
const RULE_SCOPE_BY_CATEGORY = 'Por Categorizacao';
const RULE_SCOPE_BY_INTERNAL_AREA = 'Por Area Interna';
...
export default class GestaoSLAWorkspace extends LightningElement {
    ...
    @track regrasFiltroOrigem = 'Todos';
    ...
    labels = {
        ...
        ruleScopeGlobal: ruleScopeGlobalLabel,
        ruleScopeByCategory: ruleScopeByCategoryLabel,
        ruleScopeByInternalArea: ruleScopeByInternalAreaLabel,
        ruleFilterOriginAll: ruleFilterOriginAllLabel,
        ...
        validityOpen: validityOpenLabel,
        validityNoStart: validityNoStartLabel,
        validityNoEnd: validityNoEndLabel,
        validityUntil: validityUntilLabel,
        ...
        errorRuleValidityEndBeforeStart: errorRuleValidityEndBeforeStartLabel
    };
    ...
    async loadRegrasSLA(gestaoId) {
        ...
        try {
            const result = await getRegrasSLA({
                ...
                origem: this.regrasFiltroOrigem === 'Todos' ? null : this.regrasFiltroOrigem,
            ...
            this.regrasSLA = (result?.regras || []).map((row) => ({
                ...
                vigenciaLabel: this.formatVigencia(row.vigenciaInicio, row.vigenciaFim),
                ativoLabel: row.ativo ? this.labels.categoryStatusOptionActive : this.labels.categoryStatusOptionInactive
            }));
        } catch (error) {
    ...
    get regrasEscopoOptions() {
        return [
            ...
            { label: this.labels.ruleScopeGlobal, value: RULE_SCOPE_GLOBAL },
            { label: this.labels.ruleScopeByCategory, value: RULE_SCOPE_BY_CATEGORY },
            { label: this.labels.ruleScopeByInternalArea, value: RULE_SCOPE_BY_INTERNAL_AREA }
        ];
    }
    ...
    get regraEscopoOptions() {
        ...
        if (this.regraForm?.escopoRegra === RULE_SCOPE_BY_INTERNAL_AREA) {
            options.push({ label: this.labels.ruleScopeByInternalArea, value: RULE_SCOPE_BY_INTERNAL_AREA });
        }
        return options;
    }

    get regrasOrigemOptions() {
        const values = Array.from(new Set((this.regrasSLA || []).map((r) => r.origem).filter((v) => !!v)));
        if (this.regrasFiltroOrigem !== 'Todos' && this.regrasFiltroOrigem && !values.includes(this.regrasFiltroOrigem)) {
            values.push(this.regrasFiltroOrigem);
        }
        return [{ label: this.labels.ruleFilterOriginAll, value: 'Todos' }, ...values.map((v) => ({ label: v, value: v }))];
    }

    get regraOrigemOptions() {
        const values = new Set(['Qualquer']);
        (this.regrasSLA || []).forEach((regra) => {
            if (regra.origem) {
                values.add(regra.origem);
            }
        });
        if (this.regraForm?.origem) {
            values.add(this.regraForm.origem);
        }
    ...
    get regraCategoriaRequired() {
        return this.regraForm?.escopoRegra !== RULE_SCOPE_GLOBAL;
    }

    get regraAreaRequired() {
        return this.regraForm?.escopoRegra === RULE_SCOPE_BY_INTERNAL_AREA;
    }
    ...
    openCreateRegraModal() {
        ...
        this.regraForm = {
            ...
            marcoSLAId: '',
            escopoRegra: contextCategoriaId ? RULE_SCOPE_BY_CATEGORY : RULE_SCOPE_GLOBAL,
            areaAtendimento: '',
            origem: 'Qualquer',
            tempoBaixa: null,
            tempoMedia: null,
    ...
    handleRegraInputChange(event) {
        ...
        if (field === 'escopoRegra' && value !== RULE_SCOPE_BY_INTERNAL_AREA) {
    ...
    formatVigencia(inicio, fim) {
        if (!inicio && !fim) {
            return this.labels.validityOpen;
        }
        const inicioLabel = inicio || this.labels.validityNoStart;
        const fimLabel = fim || this.labels.validityNoEnd;
        return `${inicioLabel} ${this.labels.validityUntil} ${fimLabel}`;
    }
    ...
    buildRegraPayload() {
        return {
            ...
            origem: this.regraForm.origem || 'Qualquer',
            ...
            vigenciaInicio: this.regraForm.vigenciaInicio || null,
            vigenciaFim: this.regraForm.vigenciaFim || null,
    ...
    validateRegraForm() {
        ...
        if ((request.escopoRegra === RULE_SCOPE_BY_CATEGORY || request.escopoRegra === RULE_SCOPE_BY_INTERNAL_AREA) && !request.categorizacaoId) {
            this.showToast(this.labels.commonError, this.labels.errorRuleCategoryRequired, 'error');
            return false;
        }
        if (request.escopoRegra === RULE_SCOPE_BY_INTERNAL_AREA && !request.areaAtendimento) {
        ...
        if (request.vigenciaInicio && request.vigenciaFim && request.vigenciaFim < request.vigenciaInicio) {
            this.showToast(this.labels.commonError, this.labels.errorRuleValidityEndBeforeStart, 'error');
            return false;
        }
        return true;
    }
