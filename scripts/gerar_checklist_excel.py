...
add(7, F, "Configurar Business Hours por unidade ou processo", True, CONCLUIDO,
    "Settings:BusinessHours (retrieve fresco em 21/06) + SOQL em GestaoSLA__c confirmando vínculo",
    "CORRIGIDO em 21/06: nota anterior do PROJECT_INDEX.md ('a criar na org') estava desatualizada/incorreta. Retrieve fresco de Settings:BusinessHours confirmou as 4 unidades ativas com horários reais configurados (seg-sex 07:00-22:00, sáb-dom 08:00-18:00, America/Recife): 'Atendimento Salvador', 'Atendimento Rio Grande', 'Atendimento Centro Logístico', 'Atendimento Rebocadores'. SOQL em GestaoSLA__c confirmou as 4 unidades ativas (Ativo__c=true) com BusinessHoursName__c apontando corretamente para cada Business Hours real e EntitlementProcessName__c preenchido. Nenhuma ação de criação foi necessária.")
add(7, F, "Criar Entitlement Process do Case", True, CONCLUIDO,
    "entitlementProcesses/atendimento salvador_v4, atendimento rio grande_v3, atendimento centro logistico_v3, atendimento rebocadores_v3",
    "ATUALIZADO em 21/06: versões avançaram (v2->v3 para RG/CL/Rebocadores, v2->v4 para Salvador) por conta do novo marco 'Fila Email' (ver item específico abaixo). Versões anteriores mantidas localmente para histórico.")
add(7, F, "NOVO MARCO (21/06): Criar marco único 'Tempo de Fila de Email' (tratado como Triagem/Resposta Chat — tempo único por Gestão de SLA, sem dependência de categorização)", True, CONCLUIDO,
    "GestaoSLA__c.TempoFilaEmailMinutos__c/UsaTempoFilaEmail__c + MilestoneType 'Fila Email' + MarcoSLA__c FonteTempo__c='GestaoSLA' + CaseMilestoneTriggerTimeCalculator (novo branch) + gestaoSLAWorkspace (checkboxes Usa Tempo de Triagem/Resposta Chat/Fila de Email)",
    "Solução: em vez de hardcode por unidade, o Admin define em cada Gestão de SLA, via 3 checkboxes ('Usa Tempo de Triagem?'/'Usa Tempo de Resposta Chat?'/'Usa Tempo de Fila de Email?'), quais dos 3 marcos únicos essa unidade usa — o campo numérico só aparece quando o checkbox correspondente está marcado. Ao marcar/desmarcar, o GestaoSLAService.syncMarcosUnicos cria/reativa ou desativa (nunca exclui) o MarcoSLA__c correspondente automaticamente; o motor (CaseMilestoneTriggerTimeCalculator) continua resolvendo por nome do marco, sem qualquer verificação de unidade. Migração de dados: Rio Grande/Centro Logístico/Rebocadores tinham o valor de Fila de Email armazenado por engano em TempoTriagemMinutos__c (eles nunca usaram Triagem de fato) — migrado para TempoFilaEmailMinutos__c (60/120/15 respectivamente), TempoTriagemMinutos__c zerado, MarcoSLA__c 'Triagem' renomeado para 'Fila Email' nessas 3 unidades. Salvador é a única unidade que usa os 3 marcos simultaneamente: ganhou um MarcoSLA__c novo 'Fila Email' (Triagem e Resposta Chat existentes preservados); o valor de TempoFilaEmailMinutos__c ficou intencionalmente em branco para Salvador, pendente de definição de negócio (checkbox marcado, mas sem o valor ainda — a validação do GestaoSLAService passa a exigir o valor sempre que o checkbox estiver marcado a partir de agora, pela tela). Deploy: 0Afbe00000ADwtFCAT (430 componentes, 0 erros, 88 testes). Migração de dados executada via Apex anônimo (fora do Service, deliberadamente, para permitir o estado transitório de Salvador).",
    status_overrides={"Salvador": CONCLUIDO, "Rio Grande": CONCLUIDO, "Centro Logístico": CONCLUIDO, "Rebocadores": CONCLUIDO})
...
add(7, F, "Criar Milestones de Atendimento de Área Interna por área N3", False, CONCLUIDO, "milestoneTypes/Atendimento N3 (único, genérico) + AreaParticipante__c.AreaAtendimento__c + RegrasSLACategorizacao__c.AreaAtendimento__c (per-área real)",
    "AUDITADO em 21/06: a especificação original previa um Milestone Type nativo por área (ex.: 'Atendimento de Área Interna - TI', '- Faturamento'); na implementação real existe só 1 Milestone Type nativo genérico 'Atendimento N3' (Standard, fallback/relatório). O rastreamento concorrente real por área é feito na camada Custom: cada AreaParticipante__c carrega seu próprio AreaAtendimento__c, com prazo calculado por RegrasSLACategorizacao__c.AreaAtendimento__c (EscopoRegra__c=Area Interna) — arquitetura diferente da literal do documento, porém atende ao requisito funcional (SLA concorrente por área).")
...
add(7, F, "Configurar regra para pausar SLA em Em Acompanhamento, se aplicável", False, CONCLUIDO, "AreaParticipanteSLAHelper.ETAPAS_PAUSA_SLA = {'Aguardando Cliente', 'Em Acompanhamento'} + CaseAreaParticipantePauseService.handleEtapaChanges",
    "CORRIGIDO em 21/06 (estava Parcial por engano): handleEtapaChanges já é genérico — itera sobre o Set ETAPAS_PAUSA_SLA, que já inclui 'Em Acompanhamento' desde antes desta auditoria. Pausa/retomada de Área Participante Interna e dos Milestones nativos (IsStopped) funciona igualmente para as duas etapas. Único gap real: não há teste unitário específico exercitando 'Em Acompanhamento' (só 'Aguardando Cliente' está coberto em CaseAreaParticipantePauseServiceTest) — recomenda-se duplicar um cenário de teste trocando a etapa, já que o código é o mesmo.")
...
add(7, F, "Usar campo Áreas de Atendimento Ativas para acionar milestones concorrentes", False, NAO_INICIADO, "Case.AreasAtendimentoAtivas__c (campo existe, FLS configurado em todos os profiles)",
    "GAP CONFIRMADO em 21/06 por busca exaustiva: Case.AreasAtendimentoAtivas__c não é referenciado em NENHUMA classe Apex, Flow, Trigger, LWC ou critério de Milestone do Entitlement Process (busca em todo o force-app, zero ocorrências fora de field-meta.xml/recordTypes/profiles). O campo existe mas está órfão — não aciona nada. O 'acionamento concorrente por área' real acontece de outra forma: AreaParticipanteService.addParticipation/addParticipationBulk cria uma AreaParticipante__c por área diretamente (parâmetro explícito da action do usuário), sem ler nem escrever esse campo do Case. DECISÃO TOMADA em 21/06 (ao tratar o item 0237 correlato): Área Interna é definitivamente sempre assistida (ação manual), nunca automática — portanto este campo não será integrado a um gatilho automático. Mantido como Não iniciado porque a automação descrita no item nunca foi construída (e agora, por decisão de negócio, não será); o campo pode ser candidato a aposentação numa limpeza futura, já que não tem uso funcional algum hoje.")
...
add(7, F, "Testar criação automática de Área Participante ao criar Case direto em etapa avançada", False, CONCLUIDO,
    "AreaParticipanteMilestoneSyncServiceTest.buildRecord_atendimentoN3_naoMapeado_tipoNull + syncByCaseIds_milestoneNaoMapeado_naoInsere",
    "RESOLVIDO em 21/06: investigação encontrou a causa raiz do 'Parcial' — AreaParticipanteMilestoneSyncService.TIPO_BY_MILESTONE não mapeia 'Atendimento N3', com comentário no código indicando 'decisão de negócio pendente'. Decisão tomada com o usuário: Área Interna é sempre assistida (ação manual via AreaParticipanteController.addParticipation), nunca criada automaticamente — mesmo quando o Case nasce direto numa etapa que dispara o Milestone nativo 'Atendimento N3'. Os 2 testes que comprovam esse comportamento JÁ EXISTIAM (buildRecord_atendimentoN3_naoMapeado_tipoNull, syncByCaseIds_milestoneNaoMapeado_naoInsere); só faltava formalizar a decisão. Comentário no código e nos testes atualizados de 'decisão pendente' para 'decisão definitiva'. Deploy: 0Afbe00000ADxanCAD (22 testes, 0 erros). Decisão correlata: o item 0226 (Áreas de Atendimento Ativas, marcado Não iniciado) permanece sem automação por este mesmo motivo — o campo está confirmadamente fora de uso por design, não por lacuna a ser preenchida.")
...
add(13, F, "Resolver erro de compilação org-wide na classe DataMass (bloqueia deploy de Apex em produção)", False, NAO_INICIADO,
    "DataMass, AlcadaTRGController, AlcadaTRGService, AlcadaTSVController, AlcadaTSVService, APICalloutDAO, AreaParticipanteController e outras (cadeia de dependência)",
    "DESCOBERTO em 21/06 ao validar um deploy de rotina: DataMass.remarkProductMass() não existe (método ausente/removido), e isso quebra a compilação de TODAS as classes dependentes dela na cadeia (incluindo AreaParticipanteController, que é do nosso módulo). Risco real para produção: sempre que um deploy inclui pelo menos uma classe/trigger Apex, a plataforma recompila TODO o Apex do org de destino antes de aceitar o deploy — isso vale tanto para RunLocalTests quanto para RunSpecifiedTests (o nível de teste só controla quais testes executam, não quais classes precisam compilar). Em produção não existe NoTestRun, então não há como contornar essa recompilação total quando o pacote contém Apex. Conclusão: qualquer deploy de Apex para produção vai falhar nessa etapa enquanto DataMass não for corrigida, independente de quão isolado seja o pacote sendo deployado. Ação recomendada: localizar e corrigir (ou remover, se obsoleta) a classe DataMass antes do próximo deploy de Apex em produção.")
add(13, F, "Resolver drift de FLS no perfil B2BMA Integration User (bloqueia deploy em lote de profiles)", False, NAO_INICIADO,
    "profiles/B2BMA Integration User.profile-meta.xml",
    "DESCOBERTO em 21/06 ao deployar um lote de 41 profiles: o Org rejeitou o deploy desse profile com 'You may not turn off permission View All dfsle__EOS_Type__c for this License Type' — o arquivo local não contém essa fieldPermission, mas o Org a exige para esse License Type, ou seja, o profile local está desatualizado em relação ao Org (drift). Como não é um profile relevante para os módulos em desenvolvimento (parece ligado a um pacote gerenciado tipo DealHub/B2B), foi excluído do deploy desta sessão sem impacto — mas continuará bloqueando qualquer deploy em lote de profiles (ex.: 'force-app/main/default/profiles' completo) até ser corrigido. Ação recomendada: fazer retrieve desse profile específico do Org para ressincronizar o arquivo local antes do próximo deploy que inclua todos os profiles.")
